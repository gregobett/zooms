            ...
            @foreach($todos as $todo)
                <li  wire:key="{{ $todo->id }}">
                    <div class="flex mt-4"> 
                        ...
                        <div>
                            <button class="flex-no-shrink p-1 pl-4 pr-4 ml-2 border-2 rounded-lg text-white border-red bg-red-800 hover:text-white hover:bg-red-900" wire:click="deleteTodo({{ $todo->id }})">Delete</button>
                        </div>
                    </div>
                </li>
            @endforeach
	...
	<!-- <div class="bg-white rounded shadow p-6 m-4 w-full lg:w-3/4 lg:max-w-lg">
        ...
        <ul>
            @foreach($todos as $todo)
                <li wire:key="{{ $todo->id }}">
                    <input type="checkbox" wire:click="toggleCompleted({{ $todo->id }})" {{ $todo->completed ? 'checked' : '' }}>
                    <span class="w-full text-grey-darkest">{{ $todo->title }}</span>
                    <button wire:click="deleteTodo({{ $todo->id }})" class="flex-no-shrink p-2 ml-2 border-2 rounded text-red border-red hover:text-white hover:bg-red">Remove</button>
                </li>
            @endforeach
        </ul>
